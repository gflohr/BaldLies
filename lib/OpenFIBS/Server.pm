...
sub new {
    ...
    my $logger = OpenFIBS::Logger->new (level => 0,
                                        stderr => $stderr);
...
sub __reopenLogger {
    ...
    my $logger = OpenFIBS::Logger->new (%options);
    $self->{__logger} = $logger;
...
sub __loadMessageDispatcher {
    ...
    $self->{__msg_dispatcher} = OpenFIBS::Session::MessageDispatcher->new ($logger, 
                                                                           @INC);
...
sub __openPorts {
    ...
    eval {
        ...
        foreach my $address (@{$config->{listen}}) {
            ...
            # addresses.
            push @listeners, OpenFIBS::Server::Listener->new ($address, $logger);
        }
...
sub __readConfiguration {
    ...
    if (empty $config->{dsn}) {
        ...
        my $db_file = File::Spec->catfile ($data_dir, 'openfibs.sqlite');
    ...
    if (empty $config->{pid_file}) {
        ...
        my $pid_file = File::Spec->catfile ($data_dir, 'openfibs.pid');
    ...
    if (empty $config->{logfile}) {
        ...
        my $logfile = File::Spec->catfile ($data_dir, 'openfibs.log');
...
sub getDatabase {
    ...
    my $backend = "OpenFIBS::Database::$config->{db_backend}";
...
sub __getDataDir {
    ...
    $self->{__data_dir} = File::Spec->catdir ($home_dir, 'openfibs');
...
OpenFIBS::Server - The OpenFIBS Server Module 
...
B<OpenFIBS::Server> is the heart of openfibs(1).  You should normally not
call this class, but execute openfibs(1) instead.
...
Check and write a pid file.  If another OpenFIBS process is currently running, 
